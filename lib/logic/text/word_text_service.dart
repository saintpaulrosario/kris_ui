...
class WordTextService {
  ...
  Future<Either<ErrorResponse, PageResult<WordText>>> retrive({
    required int page,
    required int size,
  }) async {
    try {
      final HttpResponse<ApiResult<PageResult<WordText>>> httpResponse =
          await _wordTextApi.retrieveAll(page: page, size: size);

      ApiResult<PageResult<WordText>> apiResult = httpResponse.data;
      if (httpResponse.response.statusCode == 200) {
        final PageResult<WordText> payload = apiResult.payload;
        return right(payload);
      } else {
        final ErrorResponse errorResponse = ErrorResponse.fromJson(
          httpResponse.response.data,

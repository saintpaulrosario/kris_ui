...
class _LanguageApi implements LanguageApi {
  ...
  Future<HttpResponse<ApiResult<List<Language>>>> retriveAll({
    ...
    final _headers = <String, dynamic>{};
    const Map<String, dynamic>? _data = null;
    final _options = _setStreamType<HttpResponse<ApiResult<List<Language>>>>(
      Options(method: 'GET', headers: _headers, extra: _extra)
          .compose(
            _dio.options,
            '/language',
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late ApiResult<List<Language>> _value;
    try {
      _value = ApiResult<List<Language>>.fromJson(
        _result.data!,
        (json) => json is List<dynamic>
            ? json
                  .map<Language>(
                    (i) => Language.fromJson(i as Map<String, dynamic>),
                  )
                  .toList()
            : List.empty(),
      );

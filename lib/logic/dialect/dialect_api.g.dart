...
class _DialectApi implements DialectApi {
  ...
  Future<HttpResponse<ApiResult<List<Dialect>>>> retriveAll({
    ...
    final _headers = <String, dynamic>{};
    const Map<String, dynamic>? _data = null;
    final _options = _setStreamType<HttpResponse<ApiResult<List<Dialect>>>>(
      Options(method: 'GET', headers: _headers, extra: _extra)
          .compose(
            _dio.options,
            '/dialect',
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late ApiResult<List<Dialect>> _value;
    try {
      _value = ApiResult<List<Dialect>>.fromJson(
        _result.data!,
        (json) => json is List<dynamic>
            ? json
                  .map<Dialect>(
                    (i) => Dialect.fromJson(i as Map<String, dynamic>),
                  )
                  .toList()
            : List.empty(),
      );
